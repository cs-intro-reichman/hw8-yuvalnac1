 ...
 *  a list of user names that s/he follows, and the list's size. */
 public class User {
    ...
    public boolean removeFollowee(String name) {
        ...
        for (int i = 0; i < fCount; i++){
            if (!foundName && name.toLowerCase().equals(follows[i].toLowerCase())) {
                foundName = true;
            } 
            if (foundName && i + 1 < maxfCount) {
    ...
    public int countMutual(User other) {
        int mutualFollowees = 0;
        for (int i = 0; i < this.fCount; i++){
            if (other.follows(this.follows[i])) mutualFollowees++;
        }
        return mutualFollowees;
    }
